...
class Fp32SoftmaxMultiHeadAttention(tf.keras.layers.MultiHeadAttention):
    ...
    Overriding ``_masked_softmax`` keeps this valid across Keras 2/3. The fp32 path is
    only taken for the unmasked, single-axis attention used here (softmax over the last,
    key, axis); anything else falls back to the stock bf16 implementation.
    """
...
class TransformerBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4, layerscale_init=1e-4, fp32_softmax=True, **kwargs):
        super().__init__(**kwargs)
