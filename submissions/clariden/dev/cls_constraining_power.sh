...
EXP_LOSS=(  "cls/vmim"   "cls/vmim_flow_fac1" "cls/vmim"           "cls/vmim_flow_fac1" )

# Fallback -- uncomment if the flow-head runs diverge (flow heads are less stable than GMMs):
# a richer 8-component GMM head (still 32-bin input via the MLP config).
# EXP_NAMES+=( "v34_resgelu_head8" ); EXP_MLP+=( "resgelu_finebins32" ); EXP_LOSS+=( "cls/vmim_head8" )
    ...
    if [ "$SCALE_CUT" = "hard_rebinned" ]; then
        ...
        srun -N1 --ntasks-per-node=1 --exclusive --cpus-per-task=288 --mem=450G \
            ...
            python "$REPOS/y3-deep-lss/deep_lss/apps/run_cls_training+evaluation.py" \
                ...
                --loss_config="$REPOS/y3-deep-lss/configs/loss/cls/vmim.yaml" \
