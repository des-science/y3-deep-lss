# bench_v12 — CLUSTERING, transformer arms
...
### The anchor is no longer `transformer/prod/clustering/maps+cls.yaml`
...
| `network.kwargs.base_embed_dim` | 32 → **64** | trunk 512 → 1024, matching lensing and combined |
...
`bench_v7_transformer`, 260 000 steps under FIXED `n_steps` — **not this budget**.
...
- Clustering-only, so nside 256 is the finest active `smooth_nside` and becomes the working resolution: 4 nested levels, not 5. `base_embed_dim: 32` therefore gives a 512 trunk, not 1024 — check the run log's `channel_dims` before quoting a width.
