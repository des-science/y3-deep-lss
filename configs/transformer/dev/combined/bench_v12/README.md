# bench_v12 — COMBINED, transformer arms
...
### The anchor is no longer `transformer/prod/combined/maps+cls.yaml`
...
`bench_v7_transformer`, 340 000 steps under FIXED `n_steps` — **not this budget**, so it is not a usable equal-wall anchor.
...
- **Budget:** 79 200 s = a **2-job `afterany` chain**. Same wall as every GCNN arm of the round.
- `masked_attention` is unavailable on the multi-resolution path (probe injection and the token mask are mutually exclusive; `build_map_encoder` raises). Clustering is injected at the level already running at nside 256.
