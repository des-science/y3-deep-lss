# bench_v12 — CLUSTERING, GCNN arms
...
- **Geometry:** base_channels **128** / pool_layers 2, trunk 512; the map branch runs at nside 256 (`smooth_nside.clustering: 256`). The residual body lands at nside 16
...
- **Anchor already on disk:** `bench_v11_simple` **is** `classic.yaml` (`config_check diff`
  reports *identical*): 124 400 steps at 3.144 it/s sustained, same 39 600 s wall. **Do not re-run it.**

## What clustering needs that combined does not

`convnext.yaml` **has never been run on this probe.** Bare ConvNeXt at the `mean` readout exists
only on combined; every ConvNeXt run on clustering (`bench_v7_full`, `v1`, `v2`,
...
|---|---|---|
| `classic` (on disk) | 3.144 **measured** | 124 400 |
| `convnext` | ~3.88 **PROJECTED** | ~154 k |
...
At ~1.23× the steps and the +7.3%-per-2× elasticity, ConvNeXt starts ~2.3% ahead before the block
